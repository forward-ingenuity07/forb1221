...
.factory('ProductsData', function($http, $q, $interval, ProductsStorage) {
    ...
    // when the response is available.
    success(function (d) {
        $http({ method: 'GET', url: 'http://www.forwardingenuity.com/accom2.php'})
    ...
    service.getLetterLimit = function() { return 50; };

    service.intervalnew = function () {

        $http({ method: 'GET', url: json, timeout: 15000 }).
 // this callback will be called asynchronously
 // when the response is available.
 success(function (d) {
     $http({ method: 'GET', url: 'http://www.forwardingenuity.com/accom.php' })
     .then(function (response) {
         window.localStorage.setItem('info', JSON.stringify(response.data));
